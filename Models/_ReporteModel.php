...
class ReporteModel extends Oracle
{
	...
	public function reporteAvanceGeneral(int $idprocesos, int $idSolucion)
	{
		...
		$query = "SELECT ST.SOLUCIONTECNOLOGICA, MS.ID_ODPE, O.NOMBRE_ODPE, COUNT(DISTINCT MS.ID_SUFRAGIO) META, 
										...
										REPLACE(ROUND(((COUNT(CCP.ID_SUFRAGIO)/2)/COUNT(MS.ID_SUFRAGIO)*100),2),',','.') DISPOSITIVOS
										...
										WHERE MS.ID_PROCESO = $this->intIdProceso ".$where."
 							 			GROUP BY ST.SOLUCIONTECNOLOGICA, MS.ID_ODPE, O.NOMBRE_ODPE, O.PRIORIDAD
	...
	public function reporteAvanceGeneralGrafica(int $idprocesos, int $idSolucion)
	{
		...
		$query = "SELECT 
							...
							REPLACE(NVL(ROUND(((COUNT(CCP.ID_SUFRAGIO)/2)/NULLIF(COUNT(MS.ID_SUFRAGIO),0)*100),4),0),',','.') DISPOSITIVOS
							FROM MESA_SUFRAGIO MS
							LEFT JOIN CONTROL_CALIDAD CCC ON MS.ID_SUFRAGIO = CCC.ID_SUFRAGIO AND CCC.ID_PROCESO = $this->intIdProceso AND CCC.ID_MATERIAL = 1 AND CCC.ID_ETAPA = 3
							LEFT JOIN CONTROL_CALIDAD CCL ON MS.ID_SUFRAGIO = CCL.ID_SUFRAGIO AND CCL.ID_PROCESO = $this->intIdProceso AND CCL.ID_MATERIAL = 2
							LEFT JOIN CONTROL_CALIDAD CCD ON MS.ID_SUFRAGIO = CCD.ID_SUFRAGIO AND CCD.ID_PROCESO = $this->intIdProceso AND CCD.ID_MATERIAL = 4
							LEFT JOIN CONTROL_CALIDAD CCR ON MS.ID_SUFRAGIO = CCR.ID_SUFRAGIO AND CCR.ID_PROCESO = $this->intIdProceso AND CCR.ID_MATERIAL = 3 
							LEFT JOIN CONTROL_CALIDAD CCN ON MS.ID_SUFRAGIO = CCN.ID_SUFRAGIO AND CCN.ID_PROCESO = $this->intIdProceso AND CCN.ID_MATERIAL = 5
							LEFT JOIN CONTROL_CALIDAD CCE ON MS.ID_SUFRAGIO = CCE.ID_SUFRAGIO AND CCE.ID_PROCESO = $this->intIdProceso AND CCE.ID_MATERIAL = 2 AND CCE.ID_ETAPA = 4
							LEFT JOIN CONTROL_CALIDAD CCP ON MS.ID_SUFRAGIO = CCP.ID_SUFRAGIO AND CCP.ID_PROCESO = $this->intIdProceso AND CCP.ID_MATERIAL = 6 --AND CCP.PAQUETE = 'USBC'
							WHERE MS.ID_PROCESO = $this->intIdProceso ".$where;
	...
	{ 
		...
		$query = " SELECT O.NOMBRE_ODPE, U.DEPARTAMENTO_UBI, U.PROVINCIA_UBI, U.DISTRITO_UBI, L.NOMBRE_LOCAL, L.DIRECCION_LOCAL,
						L.CODIGO_LOCAL,
						...
						GROUP BY O.NOMBRE_ODPE, U.DEPARTAMENTO_UBI, U.PROVINCIA_UBI, U.DISTRITO_UBI, L.NOMBRE_LOCAL, L.DIRECCION_LOCAL, L.CODIGO_LOCAL 
						ORDER BY O.NOMBRE_ODPE, U.DEPARTAMENTO_UBI, U.PROVINCIA_UBI, U.DISTRITO_UBI, L.CODIGO_LOCAL ";
	...
	{ 
		...
		$query = "  SELECT PC.TIPO_CEDULA, MS.NRO_MESA, MS.ID_SOLUCION, ST.SOLUCIONTECNOLOGICA, O.NOMBRE_ODPE, U.DEPARTAMENTO_UBI, U.PROVINCIA_UBI, U.DISTRITO_UBI, L.NOMBRE_LOCAL,
