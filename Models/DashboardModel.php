...
class DashboardModel extends Mysql
{
    ...
    public function updateUsuarioPass(int $idUsuario, string $password)
    {
        ...
        $query = "UPDATE users SET password = ?, request_password = 1 WHERE id = $this->intIdUsuario";
    ...
    public function selectCboProceso()
    {
        $query = "SELECT P.id, P.proceso, P.descripcion, TP.eleccion
                  FROM procesos P
				  INNER JOIN tipo_procesos TP ON P.id_tipo_proceso = TP.id
    ...
    /* Estas funciones todavia no se usaran ya que esto es el resumen del sistema */
