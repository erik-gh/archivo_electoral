...
//        $query = "SELECT * FROM (SELECT id_perfil, perfil, descripcion, estado, row_number() over (order by perfil) orden FROM perfil WHERE estado != 0 " . $where;
        $query = "SELECT id, perfil, descripcion, estado FROM perfiles WHERE estado != 0 ";
...
//        $query = "SELECT COUNT(*) as fila FROM perfil WHERE estado != 0 " . $where;
        $query = "SELECT COUNT(*) as fila FROM perfiles WHERE estado != 0 ";
        if (isset($_POST["search"]["value"])) {
            $query .= "AND ( perfil LIKE '%" . $_POST["search"]["value"] . "%' ";
            $query .= "OR descripcion LIKE '%" . $_POST["search"]["value"] . "%' )";
        }
    ...
    {
        $where = ($_SESSION['idPerfil'] != 1) ? ' AND id != 1 ' : '';
        $query = "SELECT * FROM perfiles WHERE estado = 1 " . $where . " ORDER BY perfil";
        $request = $this->select_all($query);
        return $request;
    }
    ...
    {
        $where = ($_SESSION['idPerfil'] != 1) ? ' AND id != 1 ' : '';
        $query = "SELECT * FROM perfiles WHERE estado = 1 " . $where . " AND id NOT IN (SELECT DISTINCT id FROM perfil_modulos) ORDER BY perfil";
        $request = $this->select_all($query);
        return $request;
    }
