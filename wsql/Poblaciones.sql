...
# Poblando proceso_odpes
# INSERT INTO archelect.proceso_odpes (id_proceso, id_odpe, created_at, updated_at)
# SELECT 1, id, now(), now() FROM odpes;
...
# Poblando mesas
SET @numero=0;
INSERT INTO archelect.mesas (nro_mesa, codigo, nro_electores, orden, id_local, id_solucion, id_consulta, created_at, updated_at)
SELECT snv.N_MESA_MADRE, '01K',snv.TOTELECMMADRE, (@numero:=@numero+1) AS orden, l.id, s.id, c.id, NOW(), now()
FROM (SELECT N_MESA_MADRE, COD_LOC, COD_ST, COD_CONSULTA, TOTELECMMADRE
FROM sufragio_nacional_vertical GROUP BY N_MESA_MADRE, COD_LOC, COD_ST, COD_CONSULTA, TOTELECMMADRE ORDER BY N_MESA_MADRE) AS snv
INNER JOIN locales l ON snv.COD_LOC = l.cod_local
INNER JOIN soluciones s on snv.COD_ST = s.id
